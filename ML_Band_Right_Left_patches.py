# ======================================================================
# WT vs KO FTIR classification — PATCH-LEVEL EVAL (LOOCV + 20-FOLD CV)
# + Cross-side train/test + Mixed (both sides)
#
# UPDATED:
#   - band features now use AMPLITUDES, not raw band maxima
#   - for each spectrum:
#       amplitude = (local band maximum) - (minimum point of that spectrum)
#   - for ratios:
#       ratio = amplitude_num / amplitude_den
#   - this is applied consistently in patch feature extraction
#
# PATCHES:
#   - Randomly sample patches per sample (PATCH_H x PATCH_W) from selected clusters
#   - Extract spectra from selected-cluster pixels within each patch
#   - Preprocess:
#       PRE_BG_SHIFT -> BG subtract -> resample -> (optional) RMieSC
#   - Build features per patch (bands_only or band_ratios)
#
# EVALUATIONS:
#   A) RIGHT→RIGHT: patch-level LOOCV  + patch-level 20-fold CV
#   B) RIGHT→LEFT : train all RIGHT patches -> test all LEFT patches (no CV)
#   C) LEFT →LEFT : patch-level LOOCV  + patch-level 20-fold CV
#   D) LEFT →RIGHT: train all LEFT patches  -> test all RIGHT patches (no CV)
#   E) MIXED (RIGHT+LEFT): patch-level LOOCV + patch-level 20-fold CV
#
# SPLIT-FIRST:
#   - band_ratios thresholds computed from TRAIN patches only inside each fold
#
# NOTE:
#   - All evaluations are patch-level (patch independence assumed; may include
#     animal/sample leakage).
# ======================================================================
...
SAVE_DIR = os.path.join(RIGHT_CUBE_DIR, "WT_KO_ML_PATCHLEVEL_F")
...
logger = logging.getLogger("WT_KO_ML_PATCHLEVEL")
...
def _reference_min_matrix(X):
    ref = np.nanmin(X, axis=1)
    ref = np.where(np.isfinite(ref), ref, np.nan)
    return ref

def _band_amplitude_topk_mean_matrix(X, wns, center, halfwidth, k=TOPK_PER_BAND):
    band_max = _band_value_topk_mean_matrix(X, wns, center, halfwidth, k=k)
    ref_min = _reference_min_matrix(X)
    amp = band_max - ref_min
...
# ============================== FEATURES (split-first thresholds) ==============================
def _compute_ratio_thresholds_training(train_band_dicts, ratio_defs):
    ...
    for rname, cfg in ratio_defs.items():
        ...
        for bdict in train_band_dicts:
            num = np.asarray(bdict[num_key], float)
            den = np.asarray(bdict[den_key], float)
...
def _predict_proba_ko(pipe, Xte1):
    try:
        proba = pipe.predict_proba(Xte1)[0]
        classes_ = pipe.named_steps["clf"].classes_
        ko_pos = int(np.where(classes_ == 1)[0][0])
        return float(proba[ko_pos])
    except Exception:
        try:
            score = float(pipe.decision_function(Xte1)[0])
    ...
    for sid in sample_names:
        ...
        for pi, (r0, c0, r1, c1) in enumerate(boxes):
            ...
            for bkey, (center, halfw) in BAND_LIBRARY.items():
                bdict[bkey] = _band_amplitude_topk_mean_matrix(
                    X_after, WN_STD, float(center), float(halfw), k=TOPK_PER_BAND
                )
...
def plot_prob_violin(y_true, y_prob, out_png, title):
    ...
    ax.scatter(np.zeros_like(wt) + 0 + j_wt, wt, s=18, alpha=0.55,
               edgecolors="black", linewidths=0.25, color=BASE_WT)
    ax.scatter(np.zeros_like(ko) + 1 + j_ko, ko, s=18, alpha=0.55,
...
def eval_patch_loocv(patches):
    ...
    for tr_idx, te_idx in loo.split(idx):
        train_p = [patches[i] for i in tr_idx]
        test_p  = patches[int(te_idx[0])]

        thresholds = None
        if FEATURE_MODE == "band_ratios":
            thresholds = _compute_ratio_thresholds_training([p["bands"] for p in train_p], BASE_RATIOS)

        Xtr, ytr, _, _, _ = _features_matrix_from_patches(train_p, thresholds)
        Xte, yte, pid_te, sid_te, _ = _features_matrix_from_patches([test_p], thresholds)
        ...
        try:
            ...
            try:
                ...
                prob_ko = float(1.0 / (1.0 + np.exp(-score)))
            except Exception:
                prob_ko = np.nan

        pids.append(pid_te[0]); sids.append(sid_te[0])
        y_true.append(int(yte[0])); y_pred.append(pred); y_prob.append(prob_ko)

    return (np.array(pids, object), np.array(sids, object),
            np.array(y_true, int), np.array(y_pred, int), np.array(y_prob, float))
...
def eval_patch_stratified_kfold(patches, n_splits=20, shuffle=True, random_state=123):
    ...
    fold_num = 0
    for tr_idx, te_idx in skf.split(idx, y_all):
        fold_num += 1
        train_p = [patches[i] for i in tr_idx]
        ...
        prob = np.array([_predict_proba_ko(pipe, Xte[i:i+1]) for i in range(Xte.shape[0])], dtype=float)

        fold_truth_probs.append((yte.astype(int), prob.astype(float)))

        pids.extend(pid_te.tolist())
        sids.extend(sid_te.tolist())
        fold_ids.extend([fold_num] * len(pid_te))
        y_true.extend(yte.tolist())
        y_pred.extend(pred.tolist())
        y_prob.extend(prob.tolist())

    return (np.array(pids, object), np.array(sids, object), np.array(fold_ids, int),
            np.array(y_true, int), np.array(y_pred, int), np.array(y_prob, float),
            fold_truth_probs)

def eval_train_all_test_all(train_patches, test_patches):
    thresholds = None
    if FEATURE_MODE == "band_ratios":
        thresholds = _compute_ratio_thresholds_training([p["bands"] for p in train_patches], BASE_RATIOS)

    Xtr, ytr, _, _, _ = _features_matrix_from_patches(train_patches, thresholds)
    Xte, yte, pids, sids, _ = _features_matrix_from_patches(test_patches, thresholds)
    ...
    pred = pipe.predict(Xte).astype(int)
    prob = np.array([_predict_proba_ko(pipe, Xte[i:i+1]) for i in range(Xte.shape[0])], dtype=float)
    return pids, sids, yte.astype(int), pred.astype(int), prob

# ============================== RUN HELPERS ==============================
def run_and_save_eval(out_dir, tag, patch_ids, sample_ids, y_true, y_pred, y_prob,
                      fold_ids=None, fold_truth_probs=None):
    os.makedirs(out_dir, exist_ok=True)

    save_patch_predictions_csv(
        os.path.join(out_dir, "patch_predictions.csv"),
        patch_ids, sample_ids, y_true, y_pred, y_prob, fold_ids=fold_ids
    )

    acc, bacc, auc, cm = report_metrics(tag, y_true, y_pred, y_prob)

    plot_confusion(
        cm, os.path.join(out_dir, "confusion_pub.png"),
...
if __name__ == "__main__":
    ...
    logger.info("PATCH %dx%d | patches/sample=%d | min cluster pixels=%d",
                PATCH_H, PATCH_W, PATCHES_PER_SAMPLE, PATCH_MIN_CLUSTER_PIXELS)
    ...
    logger.info("Band values use amplitude = local band max - spectrum minimum")
    ...
    results["A_LOOCV"] = run_and_save_eval(os.path.join(baseA, "LOOCV"),
                                          "R→R LOOCV",
                                          pids, sids, yt, yp, pr)

    with LogTimer("A RIGHT→RIGHT 20-fold"):
        pids2, sids2, folds2, yt2, yp2, pr2, fold_tp = eval_patch_stratified_kfold(
            right_patches, n_splits=CV20_FOLDS, shuffle=CV20_SHUFFLE, random_state=CV20_RANDOM_STATE
        )
    results["A_CV20"] = run_and_save_eval(os.path.join(baseA, "CV20"),
                                         "R→R 20-fold",
                                         pids2, sids2, yt2, yp2, pr2,
                                         fold_ids=folds2, fold_truth_probs=fold_tp)

    # B) RIGHT→LEFT
    baseB = os.path.join(SAVE_DIR, "B_RIGHT_LEFT")
    with LogTimer("B RIGHT→LEFT Train/Test"):
        pids, sids, yt, yp, pr = eval_train_all_test_all(right_patches, left_patches)
    results["B_TRAINTEST"] = run_and_save_eval(os.path.join(baseB, "TrainTest"),
                                               "R→L Train/Test",
                                               pids, sids, yt, yp, pr)
    ...
    results["C_LOOCV"] = run_and_save_eval(os.path.join(baseC, "LOOCV"),
                                          "L→L LOOCV",
                                          pids, sids, yt, yp, pr)

    with LogTimer("C LEFT→LEFT 20-fold"):
        pids2, sids2, folds2, yt2, yp2, pr2, fold_tp = eval_patch_stratified_kfold(
            left_patches, n_splits=CV20_FOLDS, shuffle=CV20_SHUFFLE, random_state=CV20_RANDOM_STATE
        )
    results["C_CV20"] = run_and_save_eval(os.path.join(baseC, "CV20"),
                                         "L→L 20-fold",
                                         pids2, sids2, yt2, yp2, pr2,
                                         fold_ids=folds2, fold_truth_probs=fold_tp)
    ...
    with LogTimer("D LEFT→RIGHT Train/Test"):
        pids, sids, yt, yp, pr = eval_train_all_test_all(left_patches, right_patches)
    results["D_TRAINTEST"] = run_and_save_eval(os.path.join(baseD, "TrainTest"),
                                               "L→R Train/Test",
                                               pids, sids, yt, yp, pr)
    ...
    results["E_LOOCV"] = run_and_save_eval(os.path.join(baseE, "LOOCV"),
                                          "Mixed LOOCV",
                                          pids, sids, yt, yp, pr)

    with LogTimer("E MIXED 20-fold"):
        pids2, sids2, folds2, yt2, yp2, pr2, fold_tp = eval_patch_stratified_kfold(
            mixed_patches, n_splits=CV20_FOLDS, shuffle=CV20_SHUFFLE, random_state=CV20_RANDOM_STATE
        )
    results["E_CV20"] = run_and_save_eval(os.path.join(baseE, "CV20"),
                                         "Mixed 20-fold",
                                         pids2, sids2, yt2, yp2, pr2,
                                         fold_ids=folds2, fold_truth_probs=fold_tp)

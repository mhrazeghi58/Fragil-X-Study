# -*- coding: utf-8 -*-
"""
Created on Wed Feb 18 01:44:21 2026

@author: hrazeghikondela
"""

# ======================================================================
# WT vs KO spectral analysis + overlays  (FULL UPDATED SCRIPT)
#
# UPDATED HERE:
#  - KEEP ONLY BACKGROUND SUBTRACTION (BG) as the “correction”
#  - DROP:
#      * OCTAVVS / RMieSC scattering correction (disabled + code removed)
#      * L2 normalization option (removed; only "none" + ref_area modes)
#  - SAVE RESULTS / STATS AS BOTH CSV AND EXCEL FILES
#  - USE AMPLITUDE FOR BOTH SINGLE BANDS AND RATIOS:
#      * after BG subtraction (and optional preprocessing), find the minimum
#        point of each spectrum as the reference point
#      * single band value = band max - reference minimum
#      * ratio value = (numerator band max - reference minimum) /
#                      (denominator band max - reference minimum)
#
# Keeps:
#  - resample all samples to common axis (handles Z mismatch like 421 vs 426)
#  - analysis pixels capped to MAX_PIXELS_PER_SAMPLE_ANALYSIS/sample
#  - plotting uses tiny subsample fraction
#  - optional AsLS baseline + floor shift + area-normalization
#  - ratios + single-band amplitudes
...
save_dir    = os.path.join(cube_dir, "WT_KO_bands_Over_BC_BGsub_only_1/")
...
# ===================== Pixels used =====================
MAX_PIXELS_PER_SAMPLE_ANALYSIS = 1500   # analysis cap per sample
PLOT_SUBSAMPLE_FRAC = 0.003             # plotting only
MIN_PLOT_POINTS = 200

# ===================== Preprocessing toggles =====================
APPLY_ASLS_BASELINE_BANDS  = False
APPLY_ASLS_BASELINE_RATIOS = False
BASELINE_LAM = 1e6
BASELINE_P   = 0.01
BASELINE_NIT = 10

# Thickness/intensity normalization:
# "none", "ref_area_amideI", "ref_area_amideII"
...
SHIFT_MIN_FLOOR_BANDS  = 0.
SHIFT_MIN_FLOOR_RATIOS = 0.
...
BAND_LABELS = {
    "1734":      "Abs @ 1734 cm$^{-1}$",
    "amideI":    "Amide I amplitude",
    "amideII":   "Amide II amplitude",
    "ch2":       "CH$_2$ amplitude (~1464 cm$^{-1}$)",
    "ch3":       "CH$_3$ amplitude (~1375 cm$^{-1}$)",
    "po2_1080":  "Phosphate ~1080 cm$^{-1}$ amplitude",
    "po2_1235":  "Phosphate ~1235 cm$^{-1}$ amplitude",
    "carb_1030": "Carbohydrate ~1030 cm$^{-1}$ amplitude",
    "carb_1155": "Carbohydrate ~1155 cm$^{-1}$ amplitude",
}
...
def _normalize_matrix(X, wns, mode: str):
    ...
    if mode == "ref_area_amideii":
        ...
        return X / (a + EPS)
    raise ValueError(f"Unknown norm mode (l2 removed): {mode}")
...
def _preprocess_matrix(X, wns, *, do_baseline: bool, floor: float, norm_mode: str):
...
def _collect_ratios_for_sample(sample, ratio_cfg_dict):
    ...
    for rkey, cfg in ratio_cfg_dict.items():
        n_c, n_hw = cfg["num"]
        d_c, d_hw = cfg["den"]
        band_cache[("num", rkey)] = _band_amplitude_topk_mean_matrix(X, wns, float(n_c), float(n_hw), k=TOPK_PER_BAND)
        band_cache[("den", rkey)] = _band_amplitude_topk_mean_matrix(X, wns, float(d_c), float(d_hw), k=TOPK_PER_BAND)
...
# ========================== Collect single-band amplitudes ============
def _collect_band_for_sample(sample, band_key):
    ...
    v = _band_amplitude_topk_mean_matrix(X, wns, float(center), float(halfw), k=TOPK_PER_BAND)
...
# ========================== Stats table save (CSV + Excel) =============
def save_stats_table(rows, csv_path, xlsx_path=None):
    headers = ["metric","n_wt","n_ko","median_wt","median_ko","cliffs_delta"]
    df = pd.DataFrame(rows, columns=headers)

    df.to_csv(csv_path, index=False)
    logger.info("Saved stats table CSV: %s", csv_path)

    if xlsx_path is None:
        xlsx_path = os.path.splitext(csv_path)[0] + ".xlsx"
    df.to_excel(xlsx_path, index=False)
    logger.info("Saved stats table Excel: %s", xlsx_path)
...
if __name__ == "__main__":
    ...
    logger.info("Baseline bands=%s ratios=%s | Norm bands=%s ratios=%s",
    ...
    logger.info("Both single bands and ratios use amplitude = band max - spectrum minimum")
    ...
    # ---------- SINGLE-BAND AMPLITUDES ----------
    with LogTimer("Single-band amplitude plots"):
    ...
    if stats_rows:
        save_stats_table(
            stats_rows,
            out_prefix + "_WT_vs_KO_stats.csv",
            out_prefix + "_WT_vs_KO_stats.xlsx"
        )
